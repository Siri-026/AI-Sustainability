...
gemini_api_key = "AIzaSyC3YYIxTn1YKFQriRxQ5wLcNUqwKiukFuY"
...
def greenie():
    ...
    # Just render page — no AI call here for chat messages
    return render_template("greenie.html")

@app.route('/greenie-chat', methods=['POST'])
def greenie_chat():
    data = request.get_json()
    user_message = data.get("message", "").strip()

    if not user_message:
        return jsonify({"reply": "Please type something for me to help you with!"})
    ...
    try:
        ...
        prompt = PromptTemplate(
            ...
            template="""
            You are Greenie 🌿 — a friendly chatbot who helps users with environmental and sustainability tips.
            Respond to this user message in a warm, simple, and helpful way:
            "{user_message}"
            """
        )
        ...
        app.logger.error(f"Error in /greenie-chat: {e}")
        return jsonify({"reply": "Sorry, Greenie is resting now. Please try again later! 🌿"})
